...
public class VideoEncoderFromBuffer {
    ...
    public VideoEncoderFromBuffer(int width, int height) {
        ...
        File mkDir = new File(DEBUG_FILE_NAME_BASE);
        if (!mkDir.exists())
            mkDir.mkdirs();

        File file= new File(DEBUG_FILE_NAME_BASE + this.mWidth + "x"
                + this.mHeight + ".mp4");

        String fileName =file.getAbsolutePath() ;//DEBUG_FILE_NAME_BASE + this.mWidth + "x"
               // + this.mHeight + ".mp4";
